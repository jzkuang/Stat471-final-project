...
```{r lr-coefs, echo=FALSE, message = FALSE}
source("../code/functions/plot_glmnet.R")
load("../results/ridge_fit.RData")
dummified_train <- as.data.frame(model.matrix( ~ .-1, training))
...
```{r rfimportance, fig.width = 7, fig.height= 6, out.width= "80%", fig.cap = "Plot of the variable importance using the purity based importance (on the right) and the OOB prediction based importance (on the left).", fig.align='center', echo = FALSE}
