...
```{r rr-var-import, fig.width = 5.5, fig.height= 5, out.width= "70%", fig.align= 'center', fig.cap="Feature Importance Plot for Linear Regressions", fig.pos="H", echo=FALSE}
...
We tune the boosting model by using different interaction depths for the model, 1, 2, and 3, by plotting cross-validation error against the number of trees. We then choose the model with the lowest minimum CV error, which is the interaction depth of 2. Then, we use that point to identify the minimum number of trees, which is 161 trees. The plot to identify these values can be found in Figure \@ref(fig:gbmtune). With these two tuned parameters, we build a strong predictive model using the gradient boosting method.

This tuned model can also reveal the importance of certain variables, as shown in Table \@ref(tab:gbmimportance). Similar to variable importance measures in the random forest model, we see that attendance rate is by far the most important variable for graduation rate. the other two variables in the top three are percent students with disabilities and percent English Language Learners.
...
The benefit that boosting has over random forests is the ability to make partial dependence plots to determine how these variables effect the graduation rate. As shown in Figure \@ref(fig:partialdependence), we see that attendance rate increasing corresponds to increased graduation rates, similar to how we explained in the random forest model. Higher percent students with disabilities corresponds to lower graduation rates and higher percent of student English language learners corresponds to decreased graduation rates. These two percentages reflect the socioeconomic status of the students at the school, with higher percentages likely corresponding to lower socioeconomic statuses. As a result, we might expect higher percent students with disabilities and percent of student English language learners to correspond to these decreased graduation rates.
...
Table \@ref(tab:rmse) shows the test root means squared error (RMSE) for all three models. The boosting model had the lowest test error with 6.79, followed by linear ridge regression at 6.93, and random forest model performed the worst 7.17. Overall, the magnitudes of these errors are quite small, especially due to the large range of graduation rates present in the data, and the predictive power of the three models are quite similar. In addition, these results are reasonable because we would expect the boosting model to perform the best since it requires the most tuning and the most computational power. Despite requiring more computational power, the random forest model performed worse than the linear regression model. This is likely because random forests have difficulty modeling against continuous response variables due to being a tree-based method. Consequently, it can't predict the true model for graduation rates well. 

In addition each model also had very similar identification of the most important variables for predicting graduation rates. Attendance rate was the clear most important explanatory variable in all three models. In addition, generally variables reflecting low socioeconomic status within the student demographics held high importance as well, such as percent students with free lunch, percent students with disabilities, and percent student English language learners. These variables would make intuitive sense to be the most important. Greater attendance rates lead to more student participation and learning in school and generally better graduation rates as a result. Low socioeconomic status within the school would lead to less resources for the students and the school, which would suggest low graduation rates. Total pupil counts was an interesting important variable in the random forest model, which isn't entirely intuitive. This might suggest that larger amounts of students may allow students to interact and engage more in learning with each other, possibly leading to higher graduation rates. However, these variables are all based on correlation, so these suggestions are all hypothetical.

## Takeaways

While it is true that ridge regressions, random forests, and boosting models are focused on predicting results and not discovering scientifically iron-clad relationships, they still offer insights that policymakers and educators can draw from to improve high school graduation rates across the New York state. The boosting model, in particular, can be of great use given that it beat the other models in predictive power. The top three most important variables that the boosting model identified were: attendance rates, percent of students with disabilities, and percent of students who were English language learners. As stated previously in the boosting section but repeated here for convenience, graduations rates increase as attendance rates increase, number of students with disabilities decreases (with a huge drop off at 50%), and percent of students who were English language learners decreases. None of this should be counter-intuitive to any educators (though perhaps some might not have guessed that it was these variables that proved the most important).

However, given these results seriously, the potential fixes to these problems are less clear cut and potentially even counter-intuitive. For example, if we take seriously the notion that having students at school actually causes higher graduation rates than merely correlating with high graduation rate schools, then we also need to take seriously reforms to disciplinary actions against students. Punishing students by suspending them, i.e. forcing them not to come to school, is counter-intuitively (or perhaps intuitively depending on your priors) bad. By punishing these problem students through not allowing them to come to school, administrators are actually making these students' futures worse rather than better according to our model. Our model does not point to a particular reason that this is true, but it is not difficult to come up with a rationale for why. Students who don't come to school (even if not of their own free will) are likely to make it a habit. Those who make it a habit fall behind before eventually dropping out.

Further, our report also advocates for some level of humility on the part of educators and school administrators about their capabilities. While this section has so far focused mainly on the boosting model, across all three models, it is clear that poverty decreases the chances of students graduating. Variables indicating poverty litter the top ten spots on our ranking of most important variables. This is not something that educators can tackle on their own. It is instead an issue that must be tackled by policymakers and politicians who can and should direct money towards ending childhood poverty. As we write this report, the Child Tax Credit, a credit that reduced child poverty by 40 percent according to some estimates^[If Congress Fails to Act, Monthly Child Tax Credit Payments Will Stop, Child Poverty Reductions Will Be Lost (2021). https://www.cbpp.org/research/federal-tax/if-congress-fails-to-act-monthly-child-tax-credit-payments-will-stop-child], looks to expire with no permanent replacement. Congress must act, and, if they fail to, then states must take up the mantle in their stead. 
